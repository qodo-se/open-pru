...
const readmeDoxygenPageTag = "EXAMPLES_PRU_INTC";
...
function getmakefilePruPostBuildSteps(cpu, board)
{
    ...
    return [
        " $(CG_TOOL_ROOT)/bin/hexpru --diag_wrap=off --array --array:name_prefix="+ core + "Firmware  -o "+ core.toLocaleLowerCase() + "_load_bin.h " + "intc_sys_event_" + board + "_" + cpu + "_fw_ti-pru-cgt.out; $(SED) -i '0r ${MCU_PLUS_SDK_PATH}/source/pru_io/firmware/pru_load_bin_copyright.h' "+ core.toLocaleLowerCase() + "_load_bin.h ; $(MOVE) "+ core.toLocaleLowerCase() + "_load_bin.h " + "${OPEN_PRU_PATH}/examples/intc_sys_event/firmware/"+ board + "/" +core.toLocaleLowerCase() + "_load_bin.h "
    ];
}
...
function getccsPruPostBuildSteps(cpu, board)
{
    ...
    return [
        " $(CG_TOOL_ROOT)/bin/hexpru --diag_wrap=off --array --array:name_prefix="+ core + "Firmware  -o "+ core.toLocaleLowerCase() + "_load_bin.h " + "intc_sys_event_" + board + "_" + cpu + "_fw_ti-pru-cgt.out; if ${CCS_HOST_OS} == win32 $(CCS_INSTALL_DIR)/utils/cygwin/sed -i '0r ${MCU_PLUS_SDK_PATH}/source/pru_io/firmware/pru_load_bin_copyright.h' "+ core.toLocaleLowerCase() + "_load_bin.h ; if ${CCS_HOST_OS} == linux sed -i '0r ${MCU_PLUS_SDK_PATH}/source/pru_io/firmware/pru_load_bin_copyright.h' "+ core.toLocaleLowerCase() + "_load_bin.h ;" + "if ${CCS_HOST_OS} == win32 move "+ core.toLocaleLowerCase() + "_load_bin.h " + "${OPEN_PRU_PATH}/examples/intc_sys_event/firmware/"+ board + "/" +core.toLocaleLowerCase() + "_load_bin.h; if ${CCS_HOST_OS} == linux mv "+ core.toLocaleLowerCase() + "_load_bin.h " + "${OPEN_PRU_PATH}/examples/intc_sys_event/firmware/"+ board + "/" +core.toLocaleLowerCase() + "_load_bin.h "
    ];
}
...
function getComponentProperty() {
    ...
    property.name = "intc_sys_event";
    property.isInternal = false;
    property.description = "intc_sys_event PRU Project"
